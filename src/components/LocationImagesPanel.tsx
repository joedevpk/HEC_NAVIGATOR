...
import {
  ...
  setPrimaryLocationImage,
  uploadLocationImage,
} from '@/lib/api';
import type { CampusLocation, LocationImage } from '@/lib/types';

/**
 * Galerie de photos d'une salle/d'un lieu (PROBLÈME 2 du cahier des
 * charges) : sélection depuis l'appareil, plusieurs photos, photo
 * principale, suppression, réordonnancement. Distincte de la galerie du
 * bâtiment qui contient ce lieu — mêmes interactions que
 * `BuildingImagesPanel`, appliquées à `location_images`.
 */
export function LocationImagesPanel({
  location,
  onChanged,
}: {
  ...
  const reload = async () => {
    setLoading(true);
    try {
      setImages(await getLocationImages(location.id));
      setError(null);
    } catch (err) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        await uploadLocationImage(location.id, file);
      }
      await reload();
      onChanged?.();
    } catch (err) {
  ...
  const makePrimary = async (img: LocationImage) => {
    try {
      await setPrimaryLocationImage(img);
      await reload();
      onChanged?.();
    } catch (err) {
  ...
  const remove = async (img: LocationImage) => {
    if (!confirm('Supprimer cette photo ?')) return;
    try {
      await deleteLocationImage(img);
      await reload();
      onChanged?.();
    } catch (err) {
  ...
  const move = async (index: number, direction: -1 | 1) => {
    ...
    setImages(next);
    try {
      await reorderLocationImages(next);
    } catch (err) {
    ...
    <div className="ml-4 mt-1 rounded-xl border border-dashed border-slate-200 bg-slate-50/60 p-3.5">
      ...
      {loading ? (
        ...
        <p className="mt-3 text-xs text-slate-400">
          Aucune photo propre à ce lieu. Sans photo ici, la fiche de « {location.name} »
          affiche la photo du bâtiment (si disponible).
        </p>
      ) : (
